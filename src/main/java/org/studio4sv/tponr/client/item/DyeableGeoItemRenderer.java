...
public abstract class DyeableGeoItemRenderer<T extends Item & GeoItem> extends GeoItemRenderer<T> {
    protected final Set<GeoBone> dyeableBones = new ObjectArraySet<>();
    protected BakedGeoModel lastModel = null;

    public DyeableGeoItemRenderer(GeoModel<T> model) {
        super(model);
    }
    ...
    protected abstract @NotNull Color getColorForBone(GeoBone var1);

    protected void checkBoneDyeCache(T animatable, BakedGeoModel model, float partialTick, int packedLight,
                                     int packedOverlay, float red, float green, float blue, float alpha) {
